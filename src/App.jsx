...
const layerMap = {
  ...
  's2': 'bg-img-milind',
  's10': 'bg-img-fire',
  's12': 'bg-img-spotlight',
  's-siddha-mala': 'bg-img-spotlight',
  's17': 'bg-img-legacy',
  's-indra-mala': 'bg-img-hero',
...
export default function App() {
  ...
  const [cartItems, setCartItems] = useState([
    { id: 'siddha', name: 'Siddha Mala™ Flagship (Nepali)', price: 45000, qty: 1, type: 'Mala' },
    { id: '14mukhi', name: '14 Mukhi (Deva Mani)', price: 18500, qty: 1, type: 'Rare Bead' }
  ]);
  
  // Custom global variables/hooks for Astrological details generated by AstroWizard
  const [userAstrology, setUserAstrology] = useState(null);

  const handleJourneyClick = (e, intention) => {
    e.preventDefault();
    if (intention === 'rare') {
      setSearchQuery('Rare');
      setIsSearchOpen(true);
    } else {
      // Dispatch custom event to select intention in AstroWizard
      window.dispatchEvent(new CustomEvent('rudra_select_intention', { detail: { intention } }));
      // Scroll to AstroWizard
      document.getElementById('s4')?.scrollIntoView({ behavior: 'smooth' });
    }
  };

  // We can listen to a custom event or check for global changes to sync AstroWizard results
  useEffect(() => {
    const checkAstro = () => {
      const stored = localStorage.getItem('rudra_astro_profile');
      if (stored) {
        try {
          setUserAstrology(JSON.parse(stored));
        } catch {}
      }
    };
    checkAstro();
    // Check periodically or on custom interval
    const interval = setInterval(checkAstro, 2000);
  ...
  useEffect(() => {
    ...
    // Performant Scroll Reveal using IntersectionObserver
  ...
  return (
    <>
      <Header 
        onSearchClick={() => { setIsSearchOpen(true); setIsCartOpen(false); setIsProfileOpen(false); }}
        onCartClick={() => { setIsCartOpen(true); setIsSearchOpen(false); setIsProfileOpen(false); }}
        onProfileClick={() => { setIsProfileOpen(true); setIsSearchOpen(false); setIsCartOpen(false); }}
...
<section className="dark" id="s15" data-bg="28,22,19">
  <div className="inner">
    ...
    <div style={{textAlign: 'center', marginTop: '8px'}} className="reveal">
      <a href="#" onClick={(e) => { e.preventDefault(); setIsTeamOpen(true); }} className="btn-outline">Meet The Full Team →</a>
    </div>
  </div>
</section>
...
<section className="light" id="s18" data-bg="250,246,238">
  <div className="inner">
    ...
    <div style={{textAlign: 'center'}} className="reveal">
      <a href="#" onClick={(e) => { e.preventDefault(); setIsTourOpen(true); }} className="btn-teak">Take A Virtual Tour →</a>
    </div>
  </div>
</section>
...
<section className="linen" id="s11" data-bg="232,223,200">
  <div className="inner">
    ...
    <div style={{textAlign: 'center'}} className="reveal">
      <a href="#" onClick={(e) => { e.preventDefault(); setSearchQuery("Mala"); setIsSearchOpen(true); }} className="btn-teak">Explore All Malas →</a>
    </div>
  </div>
</section>
...
<section className="dark" id="s12" data-bg="28,22,19">
  <div className="inner">
    ...
    <div className="rare-cards reveal">
      ...
      <a href="#" onClick={(e) => { e.preventDefault(); setSearchQuery("14 Mukhi"); setIsSearchOpen(true); }} className="rare-card">
      ...
      <a href="#" onClick={(e) => { e.preventDefault(); setSearchQuery("Trijuti"); setIsSearchOpen(true); }} className="rare-card">
      ...
      <a href="#" onClick={(e) => { e.preventDefault(); setSearchQuery("Gauri Shankar"); setIsSearchOpen(true); }} className="rare-card">
    ...
    <div style={{textAlign: 'center'}} className="reveal">
      <a href="#" onClick={(e) => { e.preventDefault(); setSearchQuery(""); setIsSearchOpen(true); }} className="btn-gold">Explore Rare Collections →</a>
    </div>
...
{/* ══ FOOTER ══ */}
      ...
      {isSearchOpen && (
        <div style={{
          ...
          <div style={{ maxWidth: '640px', margin: '40px auto 0', width: '100%', display: 'flex', flexDirection: 'column', height: '100%' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
              <h3 className="display" style={{ fontSize: '24px', color: 'var(--gold-lt)' }}>Sacred Catalog Search</h3>
              <button 
                onClick={() => { setIsSearchOpen(false); setSearchQuery(''); }} 
                style={{ background: 'transparent', border: 'none', color: 'var(--white)', cursor: 'pointer', display: 'flex', alignItems: 'center' }}
              >
            ...
            <div style={{ position: 'relative', marginBottom: '24px' }}>
              <input 
                ...
                placeholder="Search beads, combinations, or planetary signs..." 
                style={{
                  width: '100%',
            ...
            <div style={{ marginBottom: '20px' }}>
              <p style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.1em', color: 'rgba(255, 253, 248, 0.5)', marginBottom: '10px' }}>Trending Searches</p>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {['Siddha Mala', '1 Mukhi', 'Gauri Shankar', 'Career', 'Health', 'Protection'].map((tag) => (
                  <button 
                    key={tag}
                    onClick={() => setSearchQuery(tag)}
                    style={{
                      background: searchQuery === tag ? 'rgba(196, 154, 60, 0.2)' : 'rgba(255, 253, 248, 0.03)',
                      border: searchQuery === tag ? '1px solid var(--gold)' : '1px solid rgba(255,253,248,0.1)',
                      borderRadius: '20px',
                      padding: '6px 14px',
            ...
            <div style={{ flex: 1, overflowY: 'auto', paddingRight: '4px' }}>
              {(() => {
                const allProducts = [
                  { id: 'siddha', name: 'Siddha Mala™ Flagship (Nepali)', desc: 'The direct balance of 1-14 Mukhi beads for supreme alignments.', price: 45000, type: 'Mala', tags: ['siddha mala', 'mala', 'nepali', 'planetary', 'protection', 'career'] },
                  { id: '1mukhi', name: '1 Mukhi Kaju (Half Moon)', desc: 'Rare single furrow bead representing Lord Surya.', price: 12500, type: 'Rare Bead', tags: ['1 mukhi', 'ek mukhi', 'sun', 'surya', 'rare'] },
                  { id: 'gaurishankar', name: 'Gauri Shankar (Natural Joined)', desc: 'Shiva and Parvati fusion for harmony and connections.', price: 7500, type: 'Joined Bead', tags: ['gauri shankar', 'joined', 'harmony', 'relationships'] },
                  { id: '14mukhi', name: '14 Mukhi Deva Mani', desc: 'Awakens higher intuition, courage and aura protection.', price: 18500, type: 'Rare Bead', tags: ['14 mukhi', 'deva mani', 'saturn', 'shani', 'intuition', 'protection'] },
                  { id: 'trijuti', name: 'Trijuti Rudraksha', desc: 'Three naturally joined beads representing Brahma, Vishnu, Mahesh.', price: 35000, type: 'Collector', tags: ['trijuti', 'three', 'fused', 'auspicious', 'rare'] }
                ];
                ...
                if (filtered.length === 0) {
                  return (
                    <p style={{ textAlign: 'center', color: 'rgba(255,253,248,0.4)', marginTop: '40px', fontSize: '13px' }}>
                      No sacred combinations matched your search query. Try another term.
                    </p>
                  );
                }
                ...
                return (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    {filtered.map(p => (
                      ...
                      >
                        <div>
                          <span style={{ fontSize: '9px', textTransform: 'uppercase', color: 'var(--gold)', fontWeight: 'bold' }}>{p.type}</span>
                          <h4 style={{ color: 'var(--white)', fontSize: '14px', margin: '2px 0 4px', fontWeight: '600' }}>{p.name}</h4>
                          <p style={{ color: 'rgba(255,253,248,0.6)', fontSize: '11.5px', margin: 0 }}>{p.desc}</p>
                        </div>
                        <div style={{ textAlign: 'right', flexShrink: 0 }}>
                          <p style={{ color: 'var(--gold-lt)', fontWeight: 'bold', fontSize: '14px', margin: '0 0 6px' }}>₹{p.price.toLocaleString()}</p>
                          <button 
                            onClick={() => {
                              const existing = cartItems.find(item => item.id === p.id);
                              if (existing) {
                                setCartItems(cartItems.map(item => item.id === p.id ? { ...item, qty: item.qty + 1 } : item));
                              } else {
                                setCartItems([...cartItems, { id: p.id, name: p.name, price: p.price, qty: 1, type: p.type }]);
                              }
      ...
      {/* 🛒 Sacred Basket / Cart sliding drawer */}
      {isCartOpen && (
        <div style={{
          position: 'fixed',
          top: 0,
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', borderBottom: '1px solid rgba(196,154,60,0.1)', paddingBottom: '16px' }}>
              <h3 className="display" style={{ fontSize: '22px', color: 'var(--gold-lt)' }}>Your Sacred Basket</h3>
              <button 
            ...
            {checkoutComplete ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, textAlign: 'center', padding: '20px' }}>
                <div style={{
                  ...
                  <svg viewBox="0 0 24 24" width="32" height="32" fill="none" stroke="var(--gold)" strokeWidth="2.5"><path d="M20 6L9 17l-5-5" /></svg>
                </div>
                <h4 className="display" style={{ fontSize: '20px', color: 'var(--gold-lt)', marginBottom: '8px' }}>Divine Checkout Initiated</h4>
                <p style={{ color: 'rgba(255,253,248,0.7)', fontSize: '13px', lineHeight: '1.6', margin: '0 0 24px' }}>
                  We have securely received your request. Our senior pundits will begin coordinating the <strong>Vedic Pran-Pratishta (Consecration) Puja</strong> customized to your Gotra, Rashi, and Lagna.
                </p>
                <div style={{
                  background: 'rgba(255,253,248,0.02)',
                  ...
                  Our astrologer will call you within 2 hours
                </div>
                <button 
                  onClick={() => { setCartItems([]); setIsCartOpen(false); setCheckoutComplete(false); }}
                  className="btn-gold"
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, color: 'rgba(255,253,248,0.4)' }}>
                <svg viewBox="0 0 24 24" width="48" height="48" fill="none" stroke="currentColor" strokeWidth="1.5" style={{ marginBottom: '16px', opacity: 0.5 }}><circle cx="9" cy="21" r="1" /><circle cx="20" cy="21" r="1" /><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6" /></svg>
                <p style={{ fontSize: '14px', marginBottom: '8px' }}>Your sacred basket is currently empty.</p>
                <button 
                  onClick={() => { setIsCartOpen(false); setIsSearchOpen(true); }}
                  style={{ background: 'transparent', border: 'none', color: 'var(--gold)', fontSize: '12px', textDecoration: 'underline', cursor: 'pointer' }}
                >
                  Search beads & combinations
                </button>
              </div>
            ) : (
              <>
                <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '16px', paddingRight: '4px' }}>
                  {cartItems.map((item) => (
                    ...
                    >
                      <div>
                        <span style={{ fontSize: '8.5px', textTransform: 'uppercase', color: 'var(--gold)', fontWeight: 'bold' }}>{item.type}</span>
                        <h4 style={{ color: 'var(--white)', fontSize: '13.5px', margin: '2px 0 4px', fontWeight: '600' }}>{item.name}</h4>
                        <p style={{ color: 'var(--gold-lt)', fontSize: '12.5px', margin: 0 }}>₹{(item.price * item.qty).toLocaleString()}</p>
                      </div>
                      
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', border: '1px solid rgba(196,154,60,0.2)', borderRadius: '20px', overflow: 'hidden' }}>
                          <button 
                            onClick={() => {
                              if (item.qty > 1) {
                            ...
                            style={{ background: 'transparent', border: 'none', color: 'var(--white)', padding: '4px 10px', fontSize: '14px', cursor: 'pointer' }}
                          >
                            -
                          </button>
                          <span style={{ color: 'var(--white)', fontSize: '12.5px', minWidth: '16px', textAlign: 'center' }}>{item.qty}</span>
                          <button 
                            onClick={() => {
                              setCartItems(cartItems.map(i => i.id === item.id ? { ...i, qty: i.qty + 1 } : i));
                            }}
                        ...
                        <button 
                          ...
                          style={{ background: 'transparent', border: 'none', color: 'rgba(255,253,248,0.4)', cursor: 'pointer', display: 'flex', alignItems: 'center' }}
                        >
                ...
                <div style={{ borderTop: '1px solid rgba(196,154,60,0.1)', paddingTop: '16px', marginTop: '16px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                    <span style={{ color: 'rgba(255,253,248,0.6)', fontSize: '13px' }}>Divine Subtotal</span>
                    <span style={{ color: 'var(--gold)', fontSize: '18px', fontWeight: 'bold' }}>
                      ₹{cartItems.reduce((acc, item) => acc + (item.price * item.qty), 0).toLocaleString()}
                    </span>
                  </div>
                  <p style={{ fontSize: '10.5px', color: 'rgba(255,253,248,0.45)', lineHeight: '1.4', margin: '0 0 16px', textAlign: 'center' }}>
                    ✦ Pricing includes full customized pran-pratishta puja, authentication reports, and secure global insured shipping.
                  </p>
                  <button 
        ...
        <div style={{
          ...
          <div style={{
            ...
            <button 
              ...
              style={{ position: 'absolute', top: '16px', right: '16px', background: 'transparent', border: 'none', color: 'rgba(255,253,248,0.5)', cursor: 'pointer' }}
            >
            ...
            <div style={{ textAlign: 'center', marginBottom: '20px' }}>
              <div style={{
                ...
                <svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="var(--gold)" strokeWidth="1.8"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg>
              </div>
              <h3 className="display" style={{ fontSize: '22px', color: 'var(--gold-lt)' }}>Spiritual Matrix Profile</h3>
              <p style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.08em', color: 'rgba(255, 253, 248, 0.4)', margin: '4px 0 0' }}>Client Alignment Dashboard</p>
            </div>
            ...
            {userAstrology ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div style={{
                  ...
                  <p style={{ margin: '0 0 10px', fontSize: '14px', color: 'var(--white)', fontWeight: '600' }}>
                    ✦ Kundali Alignment: {userAstrology.name}
                  </p>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '8px', fontSize: '12px', color: 'rgba(255,253,248,0.7)', marginBottom: '12px' }}>
                    <div>Moon Sign: <strong>{userAstrology.rashi.replace('Moon Sign: ', '')}</strong></div>
                    <div>Nakshatra: <strong>{userAstrology.nakshatra}</strong></div>
                    <div style={{ gridColumn: 'span 2' }}>Lagna: <strong>{userAstrology.lagna.replace('Lagna: ', '').split(' | ')[0]}</strong></div>
                  </div>
                  <p style={{ fontSize: '11.5px', color: 'rgba(255,253,248,0.8)', lineHeight: '1.5', margin: 0, fontStyle: 'italic', borderTop: '1px solid rgba(196,154,60,0.08)', paddingTop: '8px' }}>
                    "{userAstrology.text}"
                  </p>
                </div>

                <div>
                  <h4 style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--gold)', margin: '0 0 8px' }}>Active Recommendation</h4>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,253,248,0.02)', border: '1px solid rgba(255,253,248,0.05)', borderRadius: '8px', padding: '10px 14px' }}>
                    <span style={{ fontSize: '12px', color: 'var(--white)' }}>
                      {userAstrology.intention === 'career' ? 'Siddha Mala Flagship' :
                       userAstrology.intention === 'peace' ? '5 & 9 Mukhi Peace Combo' :
                       userAstrology.intention === 'health' ? '3 & 11 Mukhi Health Combo' :
                       '8 & 9 Mukhi Protection Combo'}
                    </span>
                    <button 
                      onClick={() => {
                        const intent = userAstrology.intention;
                        const item = intent === 'career' ? { id: 'siddha', name: 'Siddha Mala™ Flagship (Nepali)', price: 45000, type: 'Mala' } :
                                     intent === 'peace' ? { id: 'peace_comb', name: 'Peace Combo (5 & 9 Mukhi)', price: 9200, type: 'Combo' } :
                                     intent === 'health' ? { id: 'health_comb', name: 'Health Combo (3 & 11 Mukhi)', price: 8500, type: 'Combo' } :
                                     { id: 'protection_comb', name: 'Protection Combo (8 & 9 Mukhi)', price: 11000, type: 'Combo' };
              ...
              <div style={{ textAlign: 'center', padding: '20px 10px' }}>
                <p style={{ color: 'rgba(255,253,248,0.6)', fontSize: '13px', lineHeight: '1.6', margin: '0 0 16px' }}>
                  You haven't generated your astrological Kundali alignment yet. Map your birth coordinates to discover custom recommendations!
                </p>
                <button 
                  onClick={() => {
                    setIsProfileOpen(false);
                  ...
                  style={{ width: '100%', justifyContent: 'center' }}
                >
                  Open Astro Kundali Wizard
                </button>
              </div>
            )}

            <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: '16px', marginTop: '16px', textAlign: 'center' }}>
              <p style={{ fontSize: '10.5px', color: 'rgba(255,253,248,0.4)', margin: 0 }}>
                Client Reference Number: <strong>RL-Client-2026-A</strong>
              </p>
            </div>
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ textAlign: 'center', marginBottom: '24px' }}>
              <span style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.12em', color: 'var(--gold)', fontWeight: 600 }}>Experiential Sourcing</span>
              <h3 className="display" style={{ fontSize: '24px', color: 'var(--white)', margin: '4px 0' }}>Spiritual Exhibition Calendar</h3>
              <p style={{ fontSize: '12px', color: 'rgba(255,253,248,0.6)', margin: 0 }}>Discover, touch, and get personalized Astro-Guidance from our experts live in your city.</p>
            </div>

            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              <div style={{ background: 'rgba(196,154,60,0.04)', border: '1px solid rgba(196,154,60,0.15)', borderRadius: '10px', padding: '14px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                  <strong style={{ color: 'var(--gold-lt)', fontSize: '14px' }}>Worli Spiritual Expo — Mumbai</strong>
                  <span style={{ fontSize: '11px', background: 'rgba(196,154,60,0.15)', color: 'var(--gold)', padding: '2px 8px', borderRadius: '4px' }}>AUG 12-15</span>
                </div>
                <p style={{ margin: 0, fontSize: '12px', color: 'rgba(255,253,248,0.8)' }}>Nehru Centre Exhibition Hall, Worli. Exclusive display of rare Mukhis (1 to 21 Mukhi) and original Nepal Siddha Malas.</p>
              </div>

              <div style={{ background: 'rgba(196,154,60,0.04)', border: '1px solid rgba(196,154,60,0.15)', borderRadius: '10px', padding: '14px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                  <strong style={{ color: 'var(--gold-lt)', fontSize: '14px' }}>Divine Heritage Sourcing — New Delhi</strong>
                  <span style={{ fontSize: '11px', background: 'rgba(196,154,60,0.15)', color: 'var(--gold)', padding: '2px 8px', borderRadius: '4px' }}>SEP 04-08</span>
                </div>
                <p style={{ margin: 0, fontSize: '12px', color: 'rgba(255,253,248,0.8)' }}>Pragati Maidan, Hall 7. Live consultations with Chief Astrologer Dr. S. K. Bhattacharya. Spot authentication tests.</p>
              </div>

              <div style={{ background: 'rgba(196,154,60,0.04)', border: '1px solid rgba(196,154,60,0.15)', borderRadius: '10px', padding: '14px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                  <strong style={{ color: 'var(--gold-lt)', fontSize: '14px' }}>Sacred Alignment Fair — Bengaluru</strong>
                  <span style={{ fontSize: '11px', background: 'rgba(196,154,60,0.15)', color: 'var(--gold)', padding: '2px 8px', borderRadius: '4px' }}>OCT 18-21</span>
                </div>
                <p style={{ margin: 0, fontSize: '12px', color: 'rgba(255,253,248,0.8)' }}>Palace Grounds, Kings Court. Vedic energization rituals performed on-site by our head Brahmins.</p>
              </div>

              <div style={{ background: 'rgba(196,154,60,0.04)', border: '1px solid rgba(196,154,60,0.15)', borderRadius: '10px', padding: '14px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                  <strong style={{ color: 'var(--gold-lt)', fontSize: '14px' }}>Vedic Arts Conclave — Dubai</strong>
                  <span style={{ fontSize: '11px', background: 'rgba(196,154,60,0.15)', color: 'var(--gold)', padding: '2px 8px', borderRadius: '4px' }}>NOV 12-15</span>
                </div>
                <p style={{ margin: 0, fontSize: '12px', color: 'rgba(255,253,248,0.8)' }}>Jumeirah Beach Towers, Sapphire Hall. International collector acquisitions and custom sacred jewelry mounting services.</p>
              </div>
            </div>

            <div style={{ marginTop: '20px', borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: '16px', textAlign: 'center' }}>
              <p style={{ fontSize: '11px', color: 'rgba(255,253,248,0.5)', margin: '0 0 12px' }}>
                All exhibitions offer **Free Entry & Free Astrological Aura Consultations** to help you choose the right beads.
              </p>
              <a href="https://wa.me/919867291461?text=Hi%2C%20I%20want%20to%20register%20for%20the%20upcoming%20spiritual%20exhibition" target="_blank" rel="noopener noreferrer" className="btn-gold" style={{ display: 'inline-flex', width: '100%', justifyContent: 'center' }}>
                Pre-Register For Free VIP Consultation On WhatsApp →
              </a>
            </div>
          </div>
        </div>
      ...
      {isPackagingOpen && (
        <div style={{
          ...
          <div style={{
            ...
            <button 
              onClick={() => setIsPackagingOpen(false)} 
              style={{ position: 'absolute', top: '16px', right: '16px', background: 'transparent', border: 'none', color: 'rgba(255,253,248,0.5)', cursor: 'pointer' }}
            >
              <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
            </button>
            ...
            <div style={{ textAlign: 'center', marginBottom: '24px' }}>
              <span style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.12em', color: 'var(--gold)', fontWeight: 600 }}>Unboxing Experience</span>
              <h3 className="display" style={{ fontSize: '24px', color: 'var(--white)', margin: '4px 0' }}>Sacred Presentation Suite</h3>
              <p style={{ fontSize: '12px', color: 'rgba(255,253,248,0.6)', margin: 0 }}>Every bead is processed with absolute purity and packed securely to preserve its high spiritual resonance.</p>
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '14px' }}>
              <div style={{ background: 'rgba(255,253,248,0.02)', border: '1px solid rgba(255,253,248,0.06)', borderRadius: '8px', padding: '12px' }}>
                <strong style={{ color: 'var(--gold)', display: 'block', fontSize: '13px', marginBottom: '4px' }}>✦ Lab Certificate</strong>
                <p style={{ margin: 0, fontSize: '11px', color: 'rgba(255,253,248,0.7)', lineHeight: 1.4 }}>Individually validated RRTL authenticity lab certificate containing non-destructive X-ray scan data of inner compartments.</p>
              </div>

              <div style={{ background: 'rgba(255,253,248,0.02)', border: '1px solid rgba(255,253,248,0.06)', borderRadius: '8px', padding: '12px' }}>
                <strong style={{ color: 'var(--gold)', display: 'block', fontSize: '13px', marginBottom: '4px' }}>✦ Vedic Energization</strong>
                <p style={{ margin: 0, fontSize: '11px', color: 'rgba(255,253,248,0.7)', lineHeight: 1.4 }}>Ritual confirmation script with photos verifying the daily Prana Pratishta ceremony performed in your name by our Brahmins.</p>
              </div>

              <div style={{ background: 'rgba(255,253,248,0.02)', border: '1px solid rgba(255,253,248,0.06)', borderRadius: '8px', padding: '12px' }}>
                <strong style={{ color: 'var(--gold)', display: 'block', fontSize: '13px', marginBottom: '4px' }}>✦ Premium Wooden Box</strong>
                <p style={{ margin: 0, fontSize: '11px', color: 'rgba(255,253,248,0.7)', lineHeight: 1.4 }}>Velvet-lined protective box crafted from seasoned, cedar wood and infused with natural aromatic sandalwood oil.</p>
              </div>

              <div style={{ background: 'rgba(255,253,248,0.02)', border: '1px solid rgba(255,253,248,0.06)', borderRadius: '8px', padding: '12px' }}>
                <strong style={{ color: 'var(--gold)', display: 'block', fontSize: '13px', marginBottom: '4px' }}>✦ Ganges Cleansing Holy Water</strong>
                <p style={{ margin: 0, fontSize: '11px', color: 'rgba(255,253,248,0.7)', lineHeight: 1.4 }}>A sealed small glass vial of pristine Gangajal sourced directly from Gangotri to cleanse your beads before wearing.</p>
              </div>

              <div style={{ background: 'rgba(255,253,248,0.02)', border: '1px solid rgba(255,253,248,0.06)', borderRadius: '8px', padding: '12px', gridColumn: 'span 2' }}>
                <strong style={{ color: 'var(--gold)', display: 'block', fontSize: '13px', marginBottom: '4px' }}>✦ Maintenance & Guidance Booklet</strong>
                <p style={{ margin: 0, fontSize: '11px', color: 'rgba(255,253,248,0.7)', lineHeight: 1.4 }}>Includes certified Sanskrit wearing mantras, correct dynamic wearing rules, and an expert care booklet with a soft natural hair cleaning brush.</p>
              </div>
            </div>
      ...
      {isTeamOpen && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ textAlign: 'center', marginBottom: '24px' }}>
              <span style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.12em', color: 'var(--gold)', fontWeight: 600 }}>The Custodians</span>
              <h3 className="display" style={{ fontSize: '24px', color: 'var(--white)', margin: '4px 0' }}>Our Spiritual Counsel & Artisans</h3>
              <p style={{ fontSize: '12px', color: 'rgba(255,253,248,0.6)', margin: 0 }}>Meet the scholars, authentic pundits and skilled weavers keeping Vedic values alive since 1999.</p>
            </div>

            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              <div style={{ display: 'flex', gap: '14px', alignItems: 'center', background: 'rgba(255,253,248,0.02)', border: '1px solid rgba(255,253,248,0.06)', borderRadius: '10px', padding: '14px' }}>
                <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: 'var(--teak)', color: 'var(--white)', display: 'flex', alignItems: 'center', justifycontent: 'center', fontSize: '18px', fontWeight: 600, fontFamily: "'Cormorant Garamond', serif", flexShrink: 0, justifyContent: 'center' }}>
                  RP
                </div>
                <div>
                  <h4 style={{ margin: '0 0 2px', fontSize: '14px', color: 'var(--gold-lt)' }}>Pandit Ram Prasad Shastri</h4>
                  <p style={{ margin: '0 0 4px', fontSize: '11px', color: 'rgba(255,253,248,0.5)', textTransform: 'uppercase' }}>Senior Vedic Scholar & Puja Ritualist</p>
                  <p style={{ margin: 0, fontSize: '12px', color: 'rgba(255,253,248,0.8)' }}>Over 30 years conducting sacred Prana Pratishta energization ceremonies inside temple complexes in accordance with Rigvedic rules.</p>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: '14px', alignItems: 'center', background: 'rgba(255,253,248,0.02)', border: '1px solid rgba(255,253,248,0.06)', borderRadius: '10px', padding: '14px' }}>
                ...
                <div>
                  <h4 style={{ margin: '0 0 2px', fontSize: '14px', color: 'var(--gold-lt)' }}>Dr. S. K. Bhattacharya</h4>
                  <p style={{ margin: '0 0 4px', fontSize: '11px', color: 'rgba(255,253,248,0.5)', textTransform: 'uppercase' }}>Chief Vedic Astrologer & Researcher</p>
                  <p style={{ margin: 0, fontSize: '12px', color: 'rgba(255,253,248,0.8)' }}>Holds a PhD in Vedic Astronomy and Jyotish Sastra. Guides our algorithmic Astro-Alignment mappings based on Nakshatra transits.</p>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: '14px', alignItems: 'center', background: 'rgba(255,253,248,0.02)', border: '1px solid rgba(255,253,248,0.06)', borderRadius: '10px', padding: '14px' }}>
                ...
                <div>
                  <h4 style={{ margin: '0 0 2px', fontSize: '14px', color: 'var(--gold-lt)' }}>Shrimati Uma Devi</h4>
                  <p style={{ margin: '0 0 4px', fontSize: '11px', color: 'rgba(255,253,248,0.5)', textTransform: 'uppercase' }}>Master Mala Artisan</p>
                  <p style={{ margin: 0, fontSize: '12px', color: 'rgba(255,253,248,0.8)' }}>Handcrafting and threading auspicious combinations for 22 years. Expert in copper thread mounting, silk string knotting and sterling silver encasing.</p>
                </div>
              </div>
            </div>
      ...
      {isTourOpen && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ textAlign: 'center', marginBottom: '24px' }}>
              <span style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.12em', color: 'var(--gold)', fontWeight: 600 }}>Experiential Tour</span>
              <h3 className="display" style={{ fontSize: '24px', color: 'var(--white)', margin: '4px 0' }}>Rudralife Head Office Tour</h3>
              <p style={{ fontSize: '12px', color: 'rgba(255,253,248,0.6)', margin: 0 }}>Explore our state-of-the-art authentication labs, consultation suites, and sacred ritual altars located in Mumbai, India.</p>
            </div>
