...
export default function AstroWizard() {
  ...
  React.useEffect(() => {
    const handleCustomIntention = (e) => {
      if (e.detail && e.detail.intention) {
        setIntention(e.detail.intention);
        setStep(1);
        setErrorMsg('');
      }
    };
    window.addEventListener('rudra_select_intention', handleCustomIntention);
    return () => window.removeEventListener('rudra_select_intention', handleCustomIntention);
  }, []);
  ...
  const handleStep2Submit = () => {
    if (!dob || !tob || !pob) {
      setErrorMsg('Please enter all birth details.');
      return;
    }
    setErrorMsg('');
    setStep(3);
  ...
  const displayAstroResult = () => {
    ...
    const astroProfile = {
      ...
      intention,
      text: recText,
      mappedAt: new Date().toLocaleDateString()
    };
    localStorage.setItem('rudra_astro_profile', JSON.stringify(astroProfile));
            ...
            <div className="wizard-step active">
              ...
              {errorMsg && (
                <div style={{
                  ...
                  <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2.5" style={{flexShrink:0}}><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
                  <span>{errorMsg}</span>
                </div>
              )}
              <button 
                ...
                onClick={() => {
                  if (intention) {
                    ...
                    setErrorMsg('Please select an intention first.');
                  }
                }}
              >
                Continue →
